  ...
  <title>Przychodnia Lekarska Online</title>
    ...
    <div class="side-nav">
		...
		<div class="container" style="overflow: auto;">
                ...
                <h1 style="margin-top: 2%; font-size:x-large">BADANIA LABORATORYJNE</h1>
				<div class ="slide"></div>
                ...
                <div class="prices_view">
                          ...
                          <?php
                            ...
                            $sql = "SELECT * FROM badania_laboratoryjne";
                            $result = $connection->query($sql);
                                    if ($result->num_rows > 0) {
                                        echo "<table>";
                                        echo<<<END
                                        <tr><th style="background-color: blueviolet;">Nazwa</th><th style="background-color: blueviolet;">Cena</th><th style="background-color: blueviolet;">Wybór</th></tr>
END;
                                        while ($row = $result->fetch_assoc()) {
                                            $new = $row["nazwa_badania"];
                                            $price =$row['cena'];
                                            $id =+ 1;
                                            echo "<tr>";
                                            echo "<td>" . $row["nazwa_badania"] . "</td>";
                                            echo "<td>" . $row["cena"] . "</td>";
                                            echo <<<HTML
                                            <form method="POST" action="basket.php">
                                            <input type="hidden" name="nazwa_badania" value= '$new'/>
                                            <input type="hidden" name="cena" value= '$price'/>
                                            <td><button class="button" type="submit" name="submit_button"> Dodaj do koszyka </button>
                                                        <input type="text" name="ilosc"class="quantity-input" style="width:20px;" value="0">
